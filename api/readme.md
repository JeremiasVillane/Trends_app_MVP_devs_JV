...
- **End-points para la autenticación de usuarios:**
  <details>
    <summary>Registro de usuarios: `POST: API_URL/auth/register`</summary>
...
- **End-points para la gestión de los datos de los USUARIOS:**
  ...
  - Feed del usuario: `GET: API_URL/user/feed/:userId/:usersType`
    ...
    **usersType** puede ser: _`student`_, `*professional*`, o `*company*`
    ...
    - Se puede calcular match entre:
      `_student_`-`_professional_`, `_professional_`-`_student_`, `_student_`-`_student_`, `_professional_`-`_professional_`      Si el`**id`** es de un `*professional`* o `*student`* y el `**usersType`** es `_company_`, va a retornar las empresas cuyos puestos de trabajo coincidan con el perfil del usuario.
    Si el `**id`** es de una `*company`\* y el `**usersType`** es `_professional`\_ o `*student*`, va a retornar a los usuarios cuyos perfiles coincidan con los puestos de trabajo de esa empresa.
      Si el `**id`\** es de un `\*job\*`, va a retornar perfiles alineados solo con ese puesto de trabajo.
    **PAGINACIÓN:\*\*
    Opcionalmente se puede añadir la \_query_ **page** que va a retornar la página indicada y **perPage** que determina la cantidad de elementos por página. Si no se indica nada por defecto muestra la página **1** y **10** elementos por página.
    Por ejemplo: `GET: API_URL/user/feed/:userId/professional?page=1`retorna la página **1** de los resultados, con **10** elementos por página.`GET: API_URL/user/feed/:userId/professional?page=3&perPage=3`
...
- **End-points para los PUESTOS DE TRABAJO:**
  ...
  - Editar un puesto: `PUT: API_URL/job/:id`
    ...
    - Se espera por _body_ un objeto así:
      ```json
      {
  ...
  - Obtener todos lo puestos abiertos por la empresa logueada: `GET: API_URL/job`
    ...
    Retorna un _array_ con los trabajos de la empresa actualmente logueada:
    ```json
    [
      {
...
- **End-points para las BÚSQUEDAS:**
  - _Búsqueda de usuarios:_ - Buscar un usuario: `GET: API_URL/search/user/:id`
    Retorna un usuario `*student*`, `*professional`* o `*company\*`, según su `**id**`      Retorna un objeto como:
      ```json
      {
      ...
      ```
    - Búsqueda avanzada de usuarios:`GET: API*URL/search/users?type=`      El`\*\*type`\** siempre tiene que estar presente (`*student*`, `*professional`* o `\_company*`).
Para aplicar los filtros se encadenan las _queries_ (que coinciden con las propiedades del objeto del usuario). No necesitan ser palabras completas; también busca por fragmentos de palabra:
`/search/users?type=student&academic*formation=universitario&academic_area=informática`
    Para buscar contenido múltiple en una \_query*, hay que separar por coma cada palabra buscada:
    `/search/users?type=student&info_goals=Elegir una carrera,Encontrar una pasantía o trabajo`
    Si no se especifica ninguna _query_ retorna todos los usuarios del `**type`\*\* seleccionado.
    **PAGINACIÓN:**
    Opcionalmente se puede añadir la _query_ **page** que va a retornar la página indicada y **perPage** que determina la cantidad de elementos por página. Si no se indica nada por defecto muestra la página **1** y **10** elementos por página.
  ...
  - _Búsqueda de trabajos:_
    ...
    - Búsqueda avanzada de trabajos: `GET: API_URL/search/jobs`
      Igual que con los usuarios, se pueden buscar trabajos aplicando filtros encadenando _queries_:
      `/search/jobs?studyArea=computer&skillsRequired=javascript`
      Si no se especifica ninguna _query_ retorna todos los puestos de trabajo.
- **End-points para la gestión de las IMÁGENES:**
  - Obtener la imagen para renderizar desde el front: `GET: API_URL/images/files/:imageName`
    Retorna la imagen según su nombre de archivo.
  - Mostrar info de todas las imágenes en la base de datos: `GET: API_URL/images`
    Retorna un _array_ como:
    ```json
    [
      {
        "id": "8df77346-5f3a-47a7-99a1-8e8bafbf321a",
        "filename": "1691759883191--imagen1.png",
        "filepath": "src\\uploads\\profiles\\1691759883191--FoodWorld-Icono.png",
        "imageUrl": "/images/files/1691759883191--FoodWorld-Icono.png",
        "isProfileImage": true,
        "createdAt": "2023-08-11T13:18:03.205Z",
        "updatedAt": "2023-08-11T13:18:03.205Z",
        "userId": "0f714cac-9393-4b0e-a0e7-300d6c4e279d",
        "companyId": null,
        "adminId": null
      },
      {
        "id": "f4767747-f899-4450-bbbe-030e896b26d4",
        "filename": "1691759885831--imagen2.png",
        "filepath": "src\\uploads\\groups\\1691759885831--FoodWorld-Icono.png",
        "imageUrl": "/images/files/1691759885831--FoodWorld-Icono.png",
        "isProfileImage": false,
        "createdAt": "2023-08-11T13:18:05.834Z",
        "updatedAt": "2023-08-11T13:18:05.834Z",
        "userId": null,
        "companyId": "0f714cac-9393-4b0e-a0e7-300d6c4e279d",
        "adminId": null
      }
    ]
    ```
    Se incluye el `**userId`** o `**companyId`\*\* de quien haya subido la imagen.
  - Mostrar info de todas las imágenes subidas por un usuario: `GET: API_URL/images/user/:userId`
  ...
  - Subir una nueva imagen: `POST: API_URL/images/upload`
    ...
    Formatos admitidos: **_jpeg, jpg, png, gif, svg_**
    Cuando la subida es exitosa responde con el _id_ y la _url_ de la imagen. Para acceder a la imagen desde el front, se tiene que anteponer la `API_URL` a la url enviada:
    ```json
    {
      "imageId": "881c0f69-84a8-4709-b79e-ccd3efbc9f5e",
      "imageUrl": "/images/files/1691757045998--imagen.png"
    }
    ```
    La url para acceder a la imagen desde el front quedaría:
    `GET: http://localhost:3001/api/v1/images/files/1691759275127--imagen.png`
  ...
  - Subir imagen de perfil: `POST: API_URL/images/profile`
    ...
    Formatos admitidos: **_jpeg, jpg, png, gif, svg_**
    Cuando la subida es exitosa responde con el _id_ de la imagen y del usuario, y la _url_ de la imagen. Para acceder a la imagen desde el front, se tiene que anteponer la `API_URL` a la url enviada:
    ```json
    {
      "imageId": "ef4ad0dd-c8f7-46fb-a0b3-b432bd90d277",
      "profileId": "0f714cac-9393-4b0e-a0e7-300d6c4e279d",
      "imageUrl": "/images/files/1691759275127--imagen.png"
    }
    ```
    La url para acceder a la imagen desde el front quedaría:
    `GET: http://localhost:3001/api/v1/images/files/1691759275127--imagen.png`
  ...
  - Subir una imagen como foto de perfil de grupo: `POST: API_URL/images/group/:groupId`
    La imagen de grupo solo puede ser modificada por el creador, un integrante con rol _moderator_ o el usuario **ADMIN**.
    Formatos admitidos: **_jpeg, jpg, png, gif, svg_**
    Cuando la subida es exitosa responde con el _id_ de la imagen y del grupo, y la _url_ de la imagen. Para acceder a la imagen desde el front, se tiene que anteponer la `API_URL` a la url enviada:
    ```json
    {
      "imageId": "c017428c-ca97-45b3-9b16-7433c63b11aa",
      "groupId": 2,
      "imageUrl": "/images/files/1691758567957--imagen.png"
    }
    ```
    La url para acceder a la imagen desde el front quedaría:
    `GET: http://localhost:3001/api/v1/images/files/1691759275127--imagen.png`
...
- **End-points para el CHAT:**
  ...
  - Mostrar los mensajes de un chat específico: `GET: API_URL/chatroom/chat/:chatId/messages`
    ...
    Retorna un objeto con los datos del remitente (`*UserSent*`) y destinatario(`*UserReceived*`), y un _array_ con los mensajes.
    ```json
    {
      "UserSent": {
        "username": "anarodriguez",
  ...
  - Enviar un nuevo mensaje: `POST: API_URL/chatroom/message`
    Espera por _body_ un objeto como:
    ```json
    {
      "sender_id": "b0565a99-6250-4c4d-9886-7e3b4a1ba3fe",
      "receiver_id": "ebf3d283-38f7-42dd-b580-ee6bb1febcd6",
  ...
  - /Editar/Eliminar un mensaje: `PUT: API_URL/chatroom/chat/:chatId/message/:messageId`
    Tanto el usuario logueado como el usuario **ADMIN** pueden acceder a este endpoint. Pero solo el propio usuario puede ver el contenido de sus mensajes
    Espera el **id** del chat y el **id** del mensaje por _params_. Y por _body_ un objeto con el contenido y el estado del mensaje (_sent_, _read_, _deleted_):
    ```json
    {
      "content": "Hi y'all!",
      "messageStatus": "sent"
    ...
    ```
    Además de editar el contenido, con este end-point se puede editar el _status_ del mensaje (**_sent_**, **_read_**, **_deleted_**).
    Si el mensaje pasó del estado **_sent_** al **_read_**, no puede volver a estar en **_sent_**.
    Si el estado es cambiado a **_deleted_**, se borra el mensaje y se retorna el texto a ser renderizado:
    ```json
    {
  ...
  - Borrar un mensaje de la base de datos: `DELETE: API_URL/chatroom/chat/:chatId/message/:messageId`
    Solo el autor del mensaje o el usuario **ADMIN** pueden acceder a este _endpoint_.
